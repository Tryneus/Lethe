...
uint32_t WindowsAtomic32::add(uint32_t value) // Itanium only?
{
  return _InterlockedExchangeAdd(&m_data, value);
}

uint32_t WindowsAtomic32::bitwiseAnd(uint32_t value) // Itanium only?
{
...
uint32_t WindowsAtomic32::compareAndExchange(uint32_t value, uint32_t compare)
{
  return InterlockedCompareExchange(&m_data, value, compare);
}
...
uint64_t WindowsAtomic64::compareAndExchange(uint64_t value, uint64_t compare)
{
  return InterlockedCompareExchange64(&m_data, value, compare);
}
